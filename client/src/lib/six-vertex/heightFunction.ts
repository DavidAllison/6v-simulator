...
import { LatticeState, Vertex, EdgeState } from './types';
...
export function calculateHeightFunction(lattice: LatticeState): HeightData {
  const { width, height, vertices } = lattice;
...
export function getHeightDifference(
  ...
  // Validate positions
  if (
    from.row < 0 || from.row >= heights.length ||
    from.col < 0 || from.col >= heights[0].length ||
    to.row < 0 || to.row >= heights.length ||
    to.col < 0 || to.col >= heights[0].length
  ) {
    return null;
  }
...
export function calculateHeightGradient(
  ...
  const gradient: Array<Array<{ dx: number; dy: number }>> = Array(numRows)
    .fill(null)
    .map(() => Array(numCols).fill(null).map(() => ({ dx: 0, dy: 0 })));
...
export function verifyHeightConsistency(
  ...
  if (heights.length !== height || heights[0].length !== width) {
    errors.push(`Dimension mismatch: lattice is ${height}x${width}, heights is ${heights.length}x${heights[0].length}`);
