      ...
      <svg width={size * cellSize} height={size * cellSize}>
        ...
        {Array.from({ length: size }).map((_, row) =>
          Array.from({ length: size }).map((_, col) => {
            ...
            const isHighlighted = highlightPos && highlightPos.row === row && highlightPos.col === col;
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.controls}>
        ...
        {' '}
        <button onClick={executeSingleFlip}>Step</button>
        {' '}
        <button onClick={() => setAutoStep(!autoStep)}>
          {autoStep ? 'Stop' : 'Auto Step'}
        </button>
        {' '}
        ...
        <div className={styles.flipSection}>
            ...
            {flipHistory.map((entry, idx) => (
              ...
              >
                <div className={styles.flipEntryHeader}>
                  <strong>Step {entry.step}:</strong> Flip {entry.direction} at ({entry.position.row}, {entry.position.col})
                </div>
          ...
          {selectedFlip && (
            <div className={styles.flipDetails}>
              ...
              <div className={styles.flipDetailsInfo}>Position: ({selectedFlip.position.row}, {selectedFlip.position.col})</div>
                  ...
                  {selectedFlip.direction === FlipDirection.Up ? (
                    <>
                      ...
                      {selectedFlip.before.upRight && (
                        <tr>
                          <td>Up-Right</td>
                          <td style={{ backgroundColor: VERTEX_COLORS[selectedFlip.before.upRight] }}>
                            {selectedFlip.before.upRight} {VERTEX_ARROWS[selectedFlip.before.upRight]}
                          </td>
                          <td style={{ backgroundColor: VERTEX_COLORS[selectedFlip.after.upRight!] }}>
                            {selectedFlip.after.upRight} {VERTEX_ARROWS[selectedFlip.after.upRight!]}
                          </td>
                        </tr>
                      )}
                    </>
                    ...
                    <>
                      ...
                      {selectedFlip.before.downLeft && (
                        <tr>
                          <td>Down-Left</td>
                          <td style={{ backgroundColor: VERTEX_COLORS[selectedFlip.before.downLeft] }}>
                            {selectedFlip.before.downLeft} {VERTEX_ARROWS[selectedFlip.before.downLeft]}
                          </td>
                          <td style={{ backgroundColor: VERTEX_COLORS[selectedFlip.after.downLeft!] }}>
                            {selectedFlip.after.downLeft} {VERTEX_ARROWS[selectedFlip.after.downLeft!]}
                          </td>
                        </tr>
                      )}
                    </>
