// Test ice rule for new DWBC implementation
import { generateDWBCHighIceValid, generateDWBCLowIceValid } from './src/lib/six-vertex/dwbcIceValid.js';
...
for (const size of sizes) {
  const high = generateDWBCHighIceValid(size);
  checkIceRule(high, `DWBC High ${size}x${size}`);
  
  const low = generateDWBCLowIceValid(size);
