...
import { OptimizedSimulation, generateDWBCHighOptimized, generateDWBCLowOptimized } from '../../src/lib/six-vertex/optimizedSimulation';
 ...
 */
function validateLatticeIceRule(state: Uint8Array | Int8Array, size: number): { valid: boolean; violations: string[] } {
        ...
        if (!validation.valid) {
          ...
          for (let idx = 0; idx < stateBefore.length; idx++) {
            if (stateBefore[idx] !== stateAfter[idx]) {
              ...
              const col = idx % size;
              console.error(`Changed: (${row},${col}) from ${NUM_TO_VERTEX_TYPE[stateBefore[idx]]} to ${NUM_TO_VERTEX_TYPE[stateAfter[idx]]}`);
            }
          }
        }
